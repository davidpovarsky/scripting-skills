...
import { TransitLoadingView, EngagementLoadingView } from "../views/loading-view"
...
const H={failed:"\u05dc\u05d0 \u05e0\u05d9\u05ea\u05df \u05dc\u05d4\u05e9\u05dc\u05d9\u05dd \u05d0\u05ea \u05d4\u05d1\u05e7\u05e9\u05d4",missing:"\u05dc\u05d0 \u05d4\u05ea\u05e7\u05d1\u05dc\u05d5 \u05e0\u05ea\u05d5\u05e0\u05d9\u05dd",location:"\u05d4\u05de\u05d9\u05e7\u05d5\u05dd \u05e9\u05dc\u05d9",live:"Live Activity \u05d4\u05d5\u05e4\u05e2\u05dc\u05d4",reminder:"\u05d4\u05d4\u05ea\u05e8\u05d0\u05d4 \u05ea\u05d5\u05d6\u05de\u05e0\u05d4"}
...
async function prepareRequest(props:Props):Promise<TransitRequest>{
  ...
  const needsLineWatch=req.action==="line_watch" && req.useCurrentLocation===true && !req.stopCode && !req.stopQuery && (!Number.isFinite(req.lat)||!Number.isFinite(req.lon))
  const needsStopWatch=req.action==="stop_watch" && req.useCurrentLocation===true && !req.stopCode && !(req.stopCodes||[]).length && !req.query && (!Number.isFinite(req.lat)||!Number.isFinite(req.lon))
  const needsFollowLocation=(req.action==="stop_watch"||req.action==="line_watch") && req.followLocation===true && (!Number.isFinite(req.lat)||!Number.isFinite(req.lon))
  ...
  if(needsNearby||needsLineWatch||needsStopWatch||needsFollowLocation||needsTripOrigin){
    const loc=await Location.requestCurrent()
    if(!loc) throw new Error("Current location unavailable")
    if(needsNearby||needsLineWatch||needsStopWatch||needsFollowLocation){req.lat=loc.latitude;req.lon=loc.longitude}
...
function EngagementActionView({action}:{action?:EngagementAction}){
  ...
  if(loading)return <EngagementLoadingView kind={action||"live"}/>
...
export default function TransitSkillView(props:Props){
  ...
  if(state.loading)return <TransitLoadingView request={props as Partial<TransitRequest>}/>
