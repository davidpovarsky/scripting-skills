...
// Query Apple Exercise Time in minutes.
...
async function main() {
  ...
  try {
    ...
    const totalMin = stats?.sumQuantity(minUnit) ?? 0
    ...
    const result: Record<string, any> = {
      ...
      formatted: formatMinutes(totalMin),
    }
    ...
    if (daily) {
      ...
      const collection = await Health.queryStatisticsCollection("appleExerciseTime", {
        startDate, endDate,
        statisticsOptions: "cumulativeSum",
      ...
      result.daily = collection.statistics()
        ...
        .map(s => {
          const mins = s.sumQuantity(minUnit) ?? 0
          return {
